...
{	TIMSK0=0x00;//отключить прерывание по совпадению с OCR0A
	...
	OCR0A=61;      // Можно быстро менять скорости UART
	TCCR0A|=(1<<WGM01);
...
int UART_receiv(void)
{
	...
	for(a=0; a<9; a++)//запись в переменную usart_r данных принятых по UART
	{
		...
		if(PINB &(PINB1==1))// если на 1 пине присутствует положительный потенциал - 	
...
int main(void)
{
	...
	while(1)
	{	UART_receiv();
		if(usart_r==0x31){PORTB|=(1<<2);}//если прислали единичку, то просто засветится светодиод
		else
		{
		PORTB|=(1<<0);
		_delay_ms(500);
